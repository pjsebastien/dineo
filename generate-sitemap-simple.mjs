...
// Lire le fichier des activités et extraire les slugs
const activitiesFileContent = fs.readFileSync('./src/data/activities.ts', 'utf-8');
const slugMatches = activitiesFileContent.matchAll(/slug:\s*['"]([^'"]+)['"]/g);
const activitySlugs = Array.from(slugMatches, match => match[1]);

console.log(`Activités trouvées: ${activitySlugs.length}`);
